...
import { Search, Download, BookOpen, AlertCircle, Book, Users, Sparkles, Mic, MicOff, PlayCircle, Headphones, Clock, FileText } from 'lucide-react';

// ============================================
// API SERVICE (Debug Version)
// ============================================
const api = {
  searchGranth: async (query: string) => {
    try {
      // Using relative path for production compatibility
      // const targetUrl = `/api/search?q=${encodeURIComponent(query)}`;
      
      // For local testing on Port 5173, fallback to direct port 5000 if needed:
      ...
      try {
        ...
        console.error("🔥 JSON Parse Failed. Server sent HTML instead of JSON.");
        return { status: 'failed', message: "Server returned Invalid Data" };
      }
...
// --- 🌟 NEW: SMART "NOT FOUND" COMPONENT ---
const NoResultsCard = ({ category }: { category: string }) => {
  const isSpeedyNote = category === 'LOOKUP';
  ...
  return (
    <div className="max-w-2xl mx-auto mt-12 text-center p-8 bg-white rounded-lg shadow-lg border-t-4 border-orange-400 animate-fade-in">
      {isSpeedyNote ? (
        // UI FOR MISSING SPEEDY NOTES
        <>
          <div className="bg-orange-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
            <FileText className="w-8 h-8 text-orange-600" />
          </div>
          <h3 className="text-2xl font-bold text-maroon-900 mb-2 font-serif">Coming Soon</h3>
          <p className="text-lg text-gray-700 font-medium">
            We are still working on adding more <span className="font-bold text-orange-600">Speedy Notes</span>.
          </p>
          <p className="text-sm text-gray-500 mt-2">
            Please check back later as we digitize more summaries.
          </p>
        </>
      ) : (
        // UI FOR MISSING SHASTRA / PRAVACHAN
        <>
          <div className="bg-maroon-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
            <Clock className="w-8 h-8 text-maroon-700" />
          </div>
          <h3 className="text-2xl font-bold text-maroon-900 mb-2 font-serif">We are Working on It</h3>
          <p className="text-lg text-gray-700 font-medium">
            Soon you will get this <span className="font-bold text-maroon-700">Shastra or Pravachan</span>.
          </p>
          <p className="text-sm text-gray-500 mt-2">
            Our team is actively scanning and verifying this content for the library.
          </p>
        </>
      )}
    </div>
  ...
  return (
    <div className="py-10 px-4 text-center">
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        <p className="text-gray-600 mb-8 font-medium">Enter a Topic, Book Name, or Gatha Code</p>
        ...
        <div className="relative shadow-md rounded-lg flex items-center bg-white border border-gray-300 focus-within:ring-2 focus-within:ring-saffron-500 overflow-hidden">
          <input
            ...
            placeholder='Type or Speak... (e.g., "Samaysar padna hai" or "S10")'
          ...
          <div className="flex items-center pr-2 gap-2">
            <button
              ...
              className={`p-3 rounded-full transition-all duration-300 ${isListening ? 'bg-red-600 text-white animate-pulse' : 'text-gray-400 hover:text-maroon-700'}`}
            >
    ...
    <div className="min-h-screen bg-cream-50 font-serif flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <div className="px-4 pb-20 pt-2">
          ...
          {aiReasoning && (
            <div className="max-w-4xl mx-auto mb-8 p-4 bg-white rounded-lg border-l-4 border-saffron-500 shadow-sm flex items-start gap-3 animate-fade-in">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-2 text-maroon-900 text-sm">
                   <div className="flex items-center gap-2">
                      <span className="font-bold bg-saffron-100 text-saffron-700 px-2 py-0.5 rounded text-xs">
                        {aiReasoning.category}
                      </span>
                   </div>
                   {aiReasoning.keywords && (
                      <span className="italic text-gray-600">
                        Searching for keywords: "{aiReasoning.keywords}"
                      </span>
                   )}
                </div>
              </div>
          ...
          {/* 🌟 THIS HANDLES THE 'NOT FOUND' CASES LOGICALLY */}
          {!loading && results.length === 0 && !error && aiReasoning && (
            <NoResultsCard category={aiReasoning.category} />
          )}
